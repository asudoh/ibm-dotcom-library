...
const regexComponentsReactPath = /@carbon[\\/]ibmdotcom-web-components[\\/]es[\\/]components-react[\\/](.*)$/;
...
class CreateReactCustomElementTypeProxyPlugin {
  ...
  apply(resolver) {
    resolver.plugin(this.source, (request, callback) => {
      request.path = request.path.replace(/(?<!carbon-web-components)[\\/]es[\\/](components|globals)[\\/]/i, '/src/$1/');
